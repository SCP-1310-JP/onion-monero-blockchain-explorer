...
MempoolStatus::read_mempool()
{
    ...
    for (size_t i = 0; i < mempool_tx_info.size(); ++i)
    {
        ...
        double payed_for_kB = SIN_AMOUNT(_tx_info.fee) / tx_size;
        ...
        last_tx.fee_str          = sineg::sin_amount_to_str(_tx_info.fee, "{:0.3f}", false);
        last_tx.payed_for_kB_str = fmt::format("{:0.4f}", payed_for_kB);
        last_tx.sin_inputs_str   = sineg::sin_amount_to_str(last_tx.sum_inputs , "{:0.3f}");
        last_tx.sin_outputs_str  = sineg::sin_amount_to_str(last_tx.sum_outputs, "{:0.3f}");
        last_tx.timestamp_str    = sineg::timestamp_to_str_gm(_tx_info.receive_time);
...
bf::path MempoolStatus::blockchain_path {"/home/mwo/.bitscp1310jp1/lmdb"};
...
sineg::MicroCore*  MempoolStatus::mcore {nullptr};
